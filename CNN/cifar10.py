...
class ResNet18CIFAR10(nn.Module):
    """ResNet-18 adapted for CIFAR-10 (32×32). Exposes `.last` for TD."""
    def __init__(self, num_classes=10):
        super().__init__()
        self.conv1 = nn.Conv2d(3, 64, 3, stride=1, padding=1, bias=False)  # CIFAR stem
...
def train_side_by_side(model_base,
    ...
    opt_main = torch.optim.Adam(model_td.parameters(),   lr=lr)
    opt_pred = torch.optim.Adam(pred_head.parameters(),  lr=tau)

    history = {"test_loss_base": [], "test_acc_base": [],
    ...
    for ep in range(1, epochs + 1):
        for x, y in train_loader:
            ...
            # predictive head learns to match current logits
            pred_logits = pred_head(zt.detach())
            L_td_head   = kl_forward(pred_logits, zt.detach(), T=T_kl)

            # TD term for model's output layer (teacher = pred_head)
            ...
            # inject σ·TD into last-layer grads
            with torch.no_grad():
...
def main():
    # identical init
    base = ResNet18CIFAR10(num_classes=10).to(device)
    td   = ResNet18CIFAR10(num_classes=10).to(device)
    ...
    plt.xlabel("Epoch"); plt.ylabel("Test Accuracy (%)"); plt.title("CIFAR-10 Test Accuracy (ResNet-18)")
    ...
    plt.xlabel("Epoch"); plt.ylabel("Test Loss (CE)"); plt.title("CIFAR-10 Test Loss (ResNet-18)")
    plt.legend(); plt.grid(True); plt.tight_layout()
    plt.show()
